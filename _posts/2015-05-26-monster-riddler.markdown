...
{% raw %}
<div class="equation" data-expr="\sum_{i=3}^{\infty} P \left( R_{C} \right)^{i-1} * P \left( R \right) * P \left( S_{U}^{i-1} \geq1 \wedge S_{C}^{i-1} \geq 1 | \left\{ M_{1},...,M_{i-1} \right\} \in \left\{ U,C \right\} \right)"></div>
...
Next, I added the expected number of common monsters slayed to each of these scenarios.  When the game ends by slaying a common monster, this is simply 1.  In the other two scenarios, this is a binomial random variable, conditioned on not all the previously slayed monsters being of a single type!  Bayes helps us reduce the expression:
{% raw %}
...
{% raw %}
<div class="equation" data-expr=" = \sum_{i=2}^{\infty} \left( \frac{5}{6} \right)^{i} * \frac{1}{6} * \left[ i * \frac{3}{5} - i * \left( \frac{3}{5} \right)^{i} \right] + \sum_{i=2}^{\infty} \left( \frac{2}{3} \right)^{i} * \frac{1}{3} * \left[ i * \frac{3}{4} - i * \left( \frac{3}{4} \right)^{i} \right] + \frac{3}{20}"></div>
{% endraw %}
